...
- `security-docs/src/proxy.ts`: Next.js Middleware — generates a per-request UUID nonce and sets the `Content-Security-Policy` header (`script-src 'nonce-…' 'strict-dynamic'`). The nonce is forwarded to page components via the `x-nonce` request header.
