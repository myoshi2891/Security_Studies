  ...
  return (
    <div className={twMerge(clsx("grid grid-cols-1 sm:grid-cols-2 gap-4 my-5", className))}>
      <div className="bg-red-500/10 border border-red-400/20 rounded-md p-5">
        ...
        <ul className="list-none flex flex-col gap-2 [&>li]:flex [&>li]:gap-2.5 [&>li]:text-[0.83rem] [&>li]:text-[#94a3b8] [&>li]:leading-[1.65] [&>li::before]:content-['✗'] [&>li::before]:text-red-500 [&>li::before]:shrink-0">
          {bad.map((item, index) => (
      ...
      <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-md p-5">
        ...
        <ul className="list-none flex flex-col gap-2 [&>li]:flex [&>li]:gap-2.5 [&>li]:text-[0.83rem] [&>li]:text-[#94a3b8] [&>li]:leading-[1.65] [&>li::before]:content-['✓'] [&>li::before]:text-emerald-500 [&>li::before]:shrink-0">
          {good.map((item, index) => (
            <li key={index}>{item}</li>
          ))}
        </ul>
